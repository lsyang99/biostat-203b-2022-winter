...
icustays_tble <- icustays_tble %>% 
  mutate(intime = as.Date(intime, format = "%Y-%m-%d %H:%M:%S")) %>%
  na.omit(intime)
  group_by(subject_id) %>% 
  arrange(intime, .by_group = TRUE)
icustays_tble[-c(5811797460, 5694947820, 5304682440, 4853903700, 7274709180), ] %>% 
  group_by(subject_id) %>% 
  arrange(intime, .by_group = TRUE) %>% 
  slice_head(n = 1) %>% 
  ungroup()
...
```{r}
# # test 1
# S_IDs <- unique(icustays_tble$subject_id)
# H_IDs <- unique(icustays_tble$hadm_id)
# 
# admissions_tble <- admissions_tble %>% 
#   filter(subject_id %in% S_IDs & hadm_id %in% H_IDs)
# admissions_tble
...
- admission hour (anything unusual?)  **未完成**
The hour start from `0` to `23`.
```{r}
admissions_tble %>% 
...
```{r}
# use semi_join
labevents0 <- read_csv("/mnt/mimiciv/1.0/hosp/labevents_filtered_itemid.csv.gz",
                       lazy = TRUE) %>% 
  semi_join(icustays_tble, by = "subject_id")
dim(labevents0) # 30198826 
labevents <- labevents0 %>%  
  filter(subject_id %in% S_IDs & 
             itemid %in% c(50912, 50971, 50983, 50902, 50882, 51221, 51301, 
                           50931, 50960, 50893))  # 16698462
semi_join()
...
```{r}
# keep the subject_id within the icustays_tble
labevents <- labevents %>%
  # pull in intime
  ...
  filter(charttime == min(charttime))  # 529247 
# make sure charttime should be >= intime of icustays_tble!!!
# 先把
# 希望使用這個 slice_min 但太慢 不要用
# merge intime into the dataframe
# sort charttime > slice_
```
4. Summarize the lab measurements by appropriate numerics and graphics. **未完成**
```{r}
# labevents_s <- labevents %>%
#   mutate(r = row_number()) %>%
...
# just use summay()
# for graphics: barplot, hist, boxplot!...
```
...
```{r}
chart <- read_csv("/mnt/mimiciv/1.0/icu/chartevents_filtered_itemid.csv.gz",
                  lazy = TRUE)
chart  # 23,679,058
chartevents_tble <- chart %>%   # 23,679,058
  filter(subject_id %in% S_IDs & 
...
chartevents_tble <- chartevents_tble %>% 
  group_by(subject_id) %>% 
  filter(charttime == min(charttime))  # 109913
```
...
**create a variable for age at admission: age_hadm = anchor_age + year(admittime) - anchor_year **
...
**Answer:**       
1. Define `thirty_day_mort`: `thirty_day_mort` = 0 if the patient didn't die or did die after 30 days upon admission.
2. Make sure `itemid` contains lab measurements *(50912, 50971, 50983, 50902, 50882, 51221, 51301, 50931, 50960, 50893)* and vital measurements *(220045, 220181, 220179, 223761, 220210)*.
Tibbles `patients_tble`, `admissions_tble`, `labevents`, and `chartevents_tble` have all been pre-processed in previous sections and only ICU patients are contained. We can use left 
```{r}
# calculate age of death
mimic_icu_cohort <- mimic_icu_cohort %>% 
  mutate(age_hadm = anchor_age + year(admittime) - anchor_year) %>% 
  filter(age_hadm > 18) %>% 
mutate(dod - admittime)

as.Date(deathtime) - as.Date(admittime)
# Dod = date of death in patient.csv


# first ICU stay
# Generate an indicator varaible: "thirty_day_mort"
admissions_tble <- admissions_tble %>% 
  mutate(mort = ifelse(is.na(deathtime), 0, (deathtime - admittime)/60/60/24),
         thirty_day_mort = ifelse(mort > 30, 1, 0)) 

# Filter admission age VVVVV
mimic_icu_cohort <- 
  left_join(icustays_tble, patients_tble, key = "subject_id") %>% 
  left_join(., admissions_tble, key = c("subject_id")) %>% 
  left_join(., labevents, key = "subject_id") %>% 
  left_join(., chartevents_tble, key = "subject_id") %>%   
  filter(anchor_age > 18)

mimic_icu_cohort  # 76,433 × 28
mimic_icu_cohort[which(mimic_icu_cohort$mort > 1), ]


# # dim(icustays_tble): 76540     8
# # dim(patients_tble): 53150     8
# # dim(labevents):     529247    4
# # dim(chart_first):   53150     8
# 
# ### semi_join
# mimic_icu_cohort <- icustays_tble %>% 
#   right_join(patients_tble, ., by = c("subject_id")) %>%
#   semi_join(admissions_tble, ., by = c("subject_id")) %>%
#   semi_join(labevents, ., by = "subject_id") %>%
#   semi_join(chartevents_tble, ., by = c("subject_id"))
#   
# mimic_icu_cohort
# mimic_icu_cohort %>% filter(anchor_age > 18)
# # Check the measurements included
# mimic_icu_cohort %>% distinct(itemid)
# mimic_icu_cohort[which]
# head(X)
# # Join all datasets
# mimic_icu_cohort <- 
#   join_all(list(icustays_tble, patients_tble, admissions_tble, labevents, 
#                 chart_first), by = "subject_id", type = "left") %>% 
#   filter(anchor_age > 18)
# 
# 
# mimic_icu_cohort %>% head
```
...
mimic_icu_cohort %>% 
  ...
  select(gender, anchor_age, ethnicity, language, marital_status) %>% 
  summarise(sum = count(ethnicity))
```
