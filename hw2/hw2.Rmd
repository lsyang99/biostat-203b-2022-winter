...
icustays_tble2 <- icustays_tble %>% 
  mutate(intime = as_datetime(intime)) 
icustays_tble %>% drop_na(subject_id)
icustays_tble[is.na(icustays_tble$intime), ]
icustays_tble2[is.na(icustays_tble2$intime), ]
...
```{r}
mimic_icu_cohort <- 
  # merge all datasets by `subject_id`
  ...
  # filter the age at admission
  filter(age_hadm > 18) %>% 
  # define `thirty_day_mort`
  mutate(mort = ifelse(is.na(deathtime), 0, (deathtime - admittime)),
         # = 1 within 30 days (equivalent to 30*24*60*60 seconds) 
         thirty_day_mort = ifelse(mort <= (30*24*60*60), 1, 0)
         ) %>%  
  # let each row be a unique subject
...
############################################################################
admissions_tble <- admissions_tble %>% 
  mutate(age_hadm = anchor_agc + year(admittime) - anchor_year) %>% 
  filter(age_hadm > 18)

mimic_icu_cohort %>% 
  mutate(deathyear = as.Date(deathtime) - as.Date(admittime))
...
# each row is a unique patient -> pivot_wider
mimic_icu_cohort <- mimic_icu_cohort %>% 
  pivot_wider(names_from = "itemid", values_from = "valuenum")
```
