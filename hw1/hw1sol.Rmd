...
4. What's the output of following bash script?
    ...
    ```
**Answer: ** It shows all the files that ends with ".gz" in the folder `core`.
...
```{bash}
# sort -u -t, -k1,1 /mnt/mimiciv/1.0/core/admissions.csv.gz
sort /mnt/mimiciv/1.0/core/admissions.csv.gz | uniq -u
```
...
`who` shows users who are currently loggin in the server.
```{bash}
who
```
...
It displays a combination of three strings (demoted in `{}`) and prints out 8 different combinations.
```{bash}
