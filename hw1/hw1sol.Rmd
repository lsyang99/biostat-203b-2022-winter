...
1. 不用做The `/mnt/mimiciv/1.0` folder on teaching server contains data sets from MIMIC-IV. Refer to the documentation <https://mimic.mit.edu/docs/iv/> for details of data files.  
    ```{bash}
...
```
Data files are stored in the `.csv.gz` format as they contain large quantity of data.

3. Briefly describe what bash commands `zcat`, `zless`, `zmore`, and `zgrep` do.
**Solution**
`zcat` views the content of a zip file - similar to what `cat` do to a file. `zcat` does not decompress the zip file.  
`zless` and `zmore` both view the the content of a zip file one line/page at a time and can go back and forth.
...
**Answer: **  
`wget` is a command used to download files to the local computer. `wget -nc` overwrites the local file when it is repeatedly downloaded.
------
